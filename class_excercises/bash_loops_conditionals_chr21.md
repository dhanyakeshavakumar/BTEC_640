...
bash-3.2$ mkdir 14_Sept
bash-3.2$ ls
14_Sept                                 Bash_best_practices_class_activity.md   Bash_best_practices_class_activity.pdf  bash_loops_conditionals_chr21.md        README
...
Step 1: Filter out the other columns that are not necessary (Column 1-8)
Step 2: Copy the columns 9 and 10
Step 3: Open a new file and paste the information that we copied column 9 and column 10
Step 4: Use find to find the exact data that we are looking for, here it is Chr21
...
```
Type your answer: It would just save the count (here it is 52142) as a file, that is not what we need
...
```
Type your answer: Use grep for searching
...
```
Type your answer: identify the delimiter in column 9 and split it using awk , in this case the delimiter is semi colon ; so we could use that to split column 9 using awk
